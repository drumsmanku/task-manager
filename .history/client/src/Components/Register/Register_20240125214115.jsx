...
function Register() {
  ...
  const signup = (event) => {
    ...
    axios.post('https://localhost:4000/register', user)
      .then(res => {
    ...
    <div className={styles.container}>
      ...
      <form className={styles.form}>
        ...
        <div className={styles.inputContainer}>
          <h4 style={{alignSelf:'start', paddingLeft:'2.2rem', fontWeight:600, marginTop:'0.5rem'}}>Create Account(Students)</h4>
          <div style={DivStyles}>
        ...
        <div style={{width:'100%', display:'flex', justifyContent:'center'}}>
          <p style={{width:'85%', textAlign:'left', fontWeight:600,fontSize:'small', marginTop:0}}>By enrolling your mobile phone number, you consent to receive automated security notifications via text message from DoubtShare. Message and data rates may apply.
          </p>
        </div>
        ...
        <div style={{width:'100%', display:'flex', paddingLeft:'2rem', paddingTop:'1rem'}}>
             <p>Are you a Tutor?</p>
             <u style={{ cursor:'pointer', marginLeft:'0.5rem'}} onClick={()=>{navigate('/register/tutor')}}>click here</u> 
        </div>
      ...
      <span style={{fontWeight:'bold', marginTop:'1rem'}}>Already have an account?<button style={{background:'none', color:'#36416A', fontSize:'medium', marginLeft:0, padding:0, width:'4rem', cursor:'pointer', border:'none', textDecoration:'underline'}} onClick={()=>{navigate('/login/student')}}>Sign in</button></span>
